...
class SprintOptimizerNew:
    ...
    def create_optimization_model(
        ...
        for t in tasks:
            task_pt = self._task_points.get(t, 0) # Use self._task_points
            M_points = max(task_pt, max_possible_daily_points, 1.0) # Use task_pt
        ...
        # 6. Task Completion:
        for t in tasks:
            self.problem += lpSum(self.points_vars[t, a, d] for a in developers for d in self._days) == self._task_points.get(t, 0) * self.task_vars[t], f"Task_Total_Points_{t}" # Use self._task_points
        ...
        for t2 in tasks:
            dependencies = self._task_dependencies.get(t2, []) # Use self._task_dependencies
            for t1 in dependencies:
                if t1 in tasks:
                    ...
                    for d_idx in range(1, num_days):
                        ...
                        self.problem += self._task_points.get(t1, 0) * work_on_t2_on_current_day - points_t1_on_previous_days <= BIG_M * (1 - self.task_vars[t1]) + BIG_M * (1 - self.task_vars[t2]), f"Precedence_{t1}_before_{t2}_Day_{current_day}" # Use self._task_points
    ...
    def solve(self) -> Tuple[Dict[str, bool], Dict[Tuple[str, str], bool], Dict[Tuple[str, str, str], bool], Dict[Tuple[str, str, str], float]]:
        ...
        # Important: Even if status is Optimal, floating point issues can occur with binary vars.
        # We should interpret binary results with a threshold.

        if status not in [LpStatusOptimal, LpStatusNotSolved]: # Allow Not Solved to inspect partial results if needed
             raise ValueError(f"Optimization failed. Status: {status_map.get(status, 'Unknown')}")

        if status == LpStatusNotSolved:
             print("Warning: Solver did not find an optimal solution. Results may be partial or inaccurate.")

        print("Optimization solving complete.")

        # Extract results, using a threshold for binary variables
        task_selection = {
            t: value(self.task_vars[t]) > 0.5 if self.task_vars[t].varValue is not None else False
            for t in self.task_vars
        }

        assignment_vars_values = {
             (t, a): value(self.assignment_vars[t, a]) if self.assignment_vars[t,a].varValue is not None else 0
             for t, a in self.assignment_vars
        }
        task_assignments = {
            (t, a): assignment_vars_values[t,a] > 0.5
            for t, a in self.assignment_vars
        }


        work_vars_values = {
             (t, a, d): value(self.work_vars[t, a, d]) if self.work_vars[t, a, d].varValue is not None else 0
             for t, a, d in self.work_vars
        }
        work_schedule = {
            (t, a, d): work_vars_values[t,a,d] > 0.5
            for t, a, d in self.work_vars
        }

        points_schedule = {
            (t, a, d): value(self.points_vars[t, a, d]) if self.points_vars[t, a, d].varValue is not None else 0
            for t, a, d in self.points_vars
        }

        # The following print statements were commented out for cleaner output,
        # but are shown here to indicate they were the commented lines.
        # print("Task Selection Results:")
        # print(task_selection)
        # print(task_assignments)
        # print(work_schedule)
        # print(points_schedule)
    ...
    def get_optimization_summary(
        ...
        # Check if problem was solved before accessing values (more robust check)
        # We check if _tasks is populated, which happens in create_optimization_model
        # and if self.problem exists.
        if not self._tasks or not self.problem:
             raise ValueError("Optimization model inputs not stored or problem not created. Call create_optimization_model first.")

        # Check if solver results are available (variables have values)
        if self.problem.status == LpStatusNotSolved or any(v.varValue is None for v in self.task_vars.values()):
             print("Warning: Solver did not find a complete solution. Summary might be incomplete or based on partial results.")
        ...
        # Sum points from the points_schedule dictionary
        total_points_scheduled = sum(points for points in points_schedule.values())
        ...
        # Extract soft constraint violation values (if problem was solved and variables have values)
        # Added check for .varValue being None
        workload_deviation_value = value(self.workload_deviation) #if self.workload_deviation and self.workload_deviation.varValue is not None else 0
        max_workload_value = value(self.max_workload) #if self.max_workload and self.max_workload.varValue is not None else 0
        min_workload_value = value(self.min_workload) #if self.min_workload and self.min_workload.varValue is not None else 0
        ...
        for t in selected_tasks:
            ...
            for dep in dependencies:
                # Check if the dependency exists in the original task list before checking its selection status
                if dep in self._tasks:
                     ...
                     # Handle dependencies that were not in the input task list (e.g., external dependencies)
                     dependency_status_list.append({
                          'dependency': dep,
                          'selected': False, # Assume not selected if not in the problem
                          'note': 'Dependency not in the list of tasks considered by the optimizer'
                     })
        ...
        return {
            ...
            'objective_value': value(self.problem.objective) if self.problem and self.problem.objective and self.problem.objective.value() is not None else 0,
...
if __name__ == "__main__":
    ...
    try:
        ...
        if selected_tasks_with_deps:
            ...
            for task in sorted(selected_tasks_with_deps.keys()):
                deps_status = selected_tasks_with_deps[task]
                ...
                if deps_status:
                    ...
                    for dep_info in sorted(deps_status, key=lambda x: x['dependency']):
                        status = "Selected" if dep_info['selected'] else "NOT Selected"
                        note = f" ({dep_info['note']})" if 'note' in dep_info else ""
                        print(f"      - {dep_info['dependency']} ({status}){note}")
                else:
        ...
        if selected_tasks_list:
            ...
            # Sort selected tasks by name for consistent output
            for t in sorted(selected_tasks_list):
                assigned_devs = [dev for dev in developers if task_assignments.get((t, dev), False)]
                print(f"  - {t} (Priority: {task_priorities.get(t,0)}, Points: {task_points.get(t,0)}) assigned to {', '.join(assigned_devs) if assigned_devs else 'No Developer Assigned (Error)'}")
        else:
            print("No tasks were selected by the optimizer.")
