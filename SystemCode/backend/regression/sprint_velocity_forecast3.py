...
import json # Import json library to output data
...
for i in range(1, num_sprints_to_forecast + 1):
    ...
    if sprint_start in future_team_size_changes:
        ...
        # print(f"--- Team size changed to {team_size_future} starting sprint on {sprint_start.date()} ---") # Removed print statements
...
# Using ISO format which is generally recommended for dates in JSON
plotting_df['start_date'] = plotting_df['start_date'].dt.strftime('%Y-%m-%d')


# --- 7. Convert Data to JSON and Print ---

# Convert the dataframe to a JSON string in 'records' format (list of dictionaries)
json_output = plotting_df.to_json(orient='records')

# Print the JSON output
print(json_output)

# --- 8. Plotting the Forecast ---
# Removed the matplotlib plotting section as the data is now output as JSON for the frontend.
